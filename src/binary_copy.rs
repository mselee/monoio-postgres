...
impl BinaryCopyInWriter {
    ...
    pub async fn write(&mut self, values: &[&(dyn ToSql)]) -> Result<(), Error> {
        ...
        I::IntoIter: ExactSizeIterator,
    {
        ...
        if self.buf.len() > 4096 {
            self.sink.send(self.buf.split().freeze()).await?;
        }
...
impl Stream for BinaryCopyOutStream {
    ...
    async fn next(&mut self) -> Option<Self::Item> {
        ...
        let has_oids = match &self.header {
            ...
            None => {
                if let Err(err) = check_remaining(&chunk, HEADER_LEN) {
                    return Some(Err(err));
                }
                ...
                if let Err(err) = check_remaining(&chunk, header_extension) {
                    return Some(Err(err));
                }
        ...
        if let Err(err) = check_remaining(&chunk, 2) {
            return Some(Err(err));
        }
        ...
        let mut ranges = vec![];
        for _ in 0..len {
            if let Err(err) = check_remaining(&chunk, 4) {
                return Some(Err(err));
            }
            ...
            if len == -1 {
                ...
                if let Err(err) = check_remaining(&chunk, len) {
                    return Some(Err(err));
                }
