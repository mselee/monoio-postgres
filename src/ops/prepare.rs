...
pub async fn prepare(
    ...
    let parameter_description = match responses.next().await.ok_or_else(Error::closed)?? {
        Message::ParameterDescription(body) => body,
        _ => return Err(Error::unexpected_message()),
    };

    let row_description = match responses.next().await.ok_or_else(Error::closed)?? {
...
pub(crate) async fn get_type(client: &Rc<InnerClient>, oid: Oid) -> Result<Type, Error> {
...
async fn get_enum_variants(client: &Rc<InnerClient>, oid: Oid) -> Result<Vec<String>, Error> {
    ...
    query::query(client, stmt, slice_iter(&[&oid]))
        .await?
        .then(|row| async move { row?.try_get(0) })
        .try_collect()
        .await
}
...
async fn get_composite_fields(client: &Rc<InnerClient>, oid: Oid) -> Result<Vec<Field>, Error> {
    ...
    let rows: Vec<Row> = query::query(client, stmt, slice_iter(&[&oid]))
        .await?
        .try_collect()
        .await?;
